...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ЭтоНовый() И Вид <> ОбщегоНазначения.ПолучитьЗначениеРеквизита(Ссылка, "Вид") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
...
Процедура ПриЗаписи(Отказ)
	...
	СтандартныеПодсистемыПереопределяемый.ОбработкаВариантовДополнительногоОтчета(ЭтотОбъект, Отказ);
...
Процедура ПередЗаписьюГлобальнойОбработки(Отказ)
	Если НЕ ДополнительныеСвойства.Свойство("КомандыОбработки") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПриЗаписиГлобальнойОбработки(Отказ)
	...
	ЗаданияДляПостановкиВОчередь = Новый Массив;
	...
	Для Каждого Задание Из ЗаданияДляПостановкиВОчередь Цикл
		СтандартныеПодсистемыПереопределяемый.ПриЗаписиЗаданияДополнительнойОбработки(Задание, РазделениеВключено);
	КонецЦикла;
...
Функция УстанавливатьПривилегированныйРежим()
	...
	Возврат РольДоступна(Метаданные.Роли.ДобавлениеИзменениеДополнительныхОтчетовИОбработок) И НЕ ПравоДоступа("Администрирование", Метаданные);
